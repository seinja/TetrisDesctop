...
public class GameBoard extends JPanel {
    ...
    // Конструктор с инициализацией помещает игровое полу внутрь окна
    ...
    // Возвращает ширину поля блока
    private int squareWidth() {
        return (int) getSize().getWidth() / BOARD_WIDTH;
    }

    // Возвращает высоту поля блока
    ...
    void start() {
        ...
        timer = new Timer(time_interval, new GameCycle());
        timer.start();
    }

    // Постановка(изменение текста) в статус баре. Сама постановка будет ниже в обработчике кнопок
    private void pause() {
    ...
    // Скидывание вниз блока при нажатии d
    ...
    //Отчиска поля (замена блоков на блоки без цвета)
    ...
    // Генерация рандомного блока и проверка не закончена ли игра
    ...
    // проверка на возможность двигаться вниз если нет то конец
    ...
    //Обработка нажатия кнопок (В андроде замени на нажатие паьцем и здвиг).
